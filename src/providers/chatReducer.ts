...
export function chatReducer(state: ChatState, action: ChatAction): ChatState {
    switch (action.type) {
        ...
        case 'TOGGLE_PLUGIN':
            return toggleTool(state, action);
        case 'TOGGLE_PLUGINS':
            return {
                ...state,
        ...
        case 'CHANGE_TEMPERATURE':
            ...
            return updateThreadList(state, newTemperature);
        case 'SET_SYSTEM_NAME':
            ...
            return updateThreadList(state, newSystemName);
        case 'SET_SYSTEM_MESSAGE':
            ...
            return updateThreadList(state, newSystemMessage);
        case 'UPDATE_CONFIG':
...
function toggleTool(state: ChatState, action: { payload: Tool }): ChatState {
    ...
    return {
        ...
        threadList: state.threadList.map((thread) =>
            thread.id === state.activeThread.id ? newToolsThread : thread,
        ),
    };
}

function updateThreadList(state: ChatState, newThread: ChatThread): ChatState {
    return {
        ...state,
        activeThread: newThread,
        threadList: state.threadList.map((thread) =>
            thread.id === state.activeThread.id ? newThread : thread,
        ),
    };
}
